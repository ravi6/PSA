model seqSkarstrom  
  ...
  parameter .Modelica.SIunits.Time bTime = 5;
...
model bed  
  ...
  .ThermoS.Uops.PSA.Adsorber adsbr(redeclare package Medium = Medium, N = NN, bedParams(Uref = 0.35, L = 0.35, dia = 0.035, voidage = 0.4, Diff = 6.0e-5, dp = 0.7e-3), Coef_p(start = cat(1, {1}, ZEROS)), Coef_Q(start = transpose({CQ1, CQ2})), Coef_y(start = transpose({CY})), Qeq(start = QStart), Q(start = QStart), y(start = yStart), yin_in(start = {0.79, 0.21}), yin_out(start = {0.79, 0.21}));
  constant Real ON = 100;
  constant Real OFF = 100 - ON;
  .ThermoS.Uops.Valves.RealValve vin(redeclare package Medium = Medium, cv = 0.1 * 1e-3 / sqrt(0.5e5), vchar = .ThermoS.Uops.Valves.Vchar.Linear, tau = 2, dpTol = 0.02);
  .ThermoS.Uops.Valves.RealValve vout(redeclare package Medium = Medium, cv = 0.933 * 0.5 * 1.15 * 1.1 * 5e-6 / sqrt(0.5e5), vchar = .ThermoS.Uops.Valves.Vchar.Linear, tau = 2, dpTol = 0.02);
  .ThermoS.Uops.Valves.RealValve vblow(redeclare package Medium = Medium, cv = 0.4 * 0.1 * 1.0e-3 / sqrt(0.5e5), vchar = .ThermoS.Uops.Valves.Vchar.Linear, tau = 2, dpTol = 0.02);
  constant Integer CycleTime = 100;
  seqSkarstrom seq(pTime = 0.3 * CycleTime, fTime = 0.2 * CycleTime, bTime = 0.3 * CycleTime, puTime = 0.2 * CycleTime);
  .ThermoS.Uops.Tanks.portMixer inNode(redeclare package Medium = Medium, vol = 5e-6, N = 3);
  .ThermoS.Uops.Tanks.portMixer outNode(redeclare package Medium = Medium, vol = 5e-6, N = 3);
initial equation
  ...
  for m in 1:adsbr.N loop
    adsbr.Q[m, 1] = 11.173 * 0.5;
    adsbr.Q[m, 2] = 0.7266 * 0.5;
  end for;
  for m in 1:adsbr.N - 2 loop
    adsbr.p[m] = 1;
  end for;
  inNode.T = 300;
  inNode.p = 1e5;
  inNode.Xi = Medium.reference_X[1:Medium.nXi];
  outNode.T = 300;
  outNode.p = 1e5;
  outNode.Xi = Medium.reference_X[1:Medium.nXi];
equation
...
model plant  
  package Medium = ThermoS.Media.MyAir(MassFlowRate(start = 0, nominal = 1e-5, min = -2e-3, max = 2e-3));
  constant Medium.MassFraction[Medium.nXi] Xstart = Medium.reference_X[1:Medium.nXi];
  constant Real ON = 100;
  constant Real OFF = 100 - ON;
  .ThermoS.Uops.Reservoir src(redeclare package Medium = Medium, p = 1.5e5, T = 300, Xi = Xstart);
  .ThermoS.Uops.Reservoir sink(redeclare package Medium = Medium, p = 1e5, T = 300, Xi = Xstart);
  .ThermoS.Uops.Reservoir atm(redeclare package Medium = Medium, p = 1e5, T = 300, Xi = Xstart);
  bed bedA;
  bed bedB;
  .ThermoS.Uops.Valves.RealValve vpurge(redeclare package Medium = Medium, cv = 2 * 0.933 * 0.5 * 10 * 2.5e-6 / sqrt(0.5e5), vchar = .ThermoS.Uops.Valves.Vchar.Linear, tau = 2, dpTol = 0.02);
initial equation
  bedA.vin.po = OFF;
  bedA.vout.po = ON;
  bedA.vblow.po = OFF;
  bedA.seq.pressurizing = false;
  bedA.seq.feeding = true;
  bedA.seq.purging = false;
  bedA.seq.blowing = false;
  bedB.vin.po = OFF;
  bedB.vout.po = OFF;
  bedB.vblow.po = ON;
  bedB.seq.pressurizing = false;
  bedB.seq.feeding = false;
  bedB.seq.purging = true;
  bedB.seq.blowing = false;
  vpurge.po = ON;
equation
...
package ThermoS  "A Modelica Package for Process Simulations" 
  package Uops  "Unit Operations in ThermoS Package" 
    package Valves  "Contains Valves and its interfaces " 
      ...
      equation
        ...
        if Compressible then
          inlet.m_flow = cv * max(0, charF) * sqrt(max(0, med.d)) * sqrt(max(inlet.p, outlet.p)) * sign(inlet.p - outlet.p) * .Modelica.Fluid.Utilities.regRoot(1 - max(prat, 0.5), dpTol);
        else
    ...
    package PSA  "Pressure Swing Adsorption Pakcage" 
      partial model partAdsorber  
        ...
        type Coef = Real(min = -200, max = 200);
      ...
      equation
        zs = z;
        p = Coef_p * vT;
        u = -1.0 / bedParams.Kappa * (Coef_p * vTz);
        u = Coef_u * vT;
        ...
        for n in 1:Nc loop
          for m in 1:N loop
            Qeq[m, n] = bedParams.Qs[n] * (bedParams.B[n] * p[m] * y[m, n]) / (bedParams.Tb + p[m] * sum(bedParams.B[j] * y[m, j] for j in 1:Nc));
            S[m, n] = bedParams.Km[n] * (Qeq[m, n] - Q[m, n]);
          end for;
        end for;
        for n in 1:Nc - 1 loop
          for m in 1:N - 2 loop
            p[m] * (der(Coef_y[:, n]) * vT[:, m] + u[m] * Coef_y[:, n] * vTz[:, m]) - 1.0 / bedParams.Pe * (p[m] * Coef_y[:, n] * vTzz[:, m] + y[m, n] * Coef_p * vTzz[:, m] + 2 * (Coef_p * vTz[:, m]) * (Coef_y[:, n] * vTz[:, m])) + (S[m, n] - y[m, n] * sum(S[m, j] for j in 1:Nc)) * bedParams.Epsilon * bedParams.Tb = 0;
          end for;
        end for;
        for m in 1:N - 2 loop
          der(Coef_p) * vT[:, m] + u[m] * Coef_p * vTz[:, m] + p[m] * Coef_u * vTz[:, m] + bedParams.Epsilon * bedParams.Tb * sum(S[m, j] for j in 1:Nc) = 0;
        end for;
        for n in 1:Nc loop
          for m in 1:N loop
        ...
        for n in 1:Nc - 1 loop
          Coef_y[:, n] * vTz[:, N - 1] = bedParams.Pe * max(u[N - 1], 0) * (y[N - 1, n] - yin_in[n]);
          Coef_y[:, n] * vTz[:, N] = bedParams.Pe * min(u[N], 0) * (y[N, n] - yin_out[n]);
        end for;
        Coef_p[:] * vT[:, N - 1] = p_in;
        Coef_p[:] * vT[:, N] = p_out;
      end partAdsorber;
      ...
      record bedParamsRec  
        ...
        Real L = 0.25;
        Real dia = 0.2;
        Real[Nc] qs = {5.26, 5.26};
        Real[Nc] hk = {14.8, 4.7};
        Real[Nc] k_m = {19.7, 62.0};
        Real csArea = 3.147 * dia * dia / 4;
        Real Cref = Pref / (R * Tref);
        Real[Nc] b = hk ./ qs;
        Real Kozney = 180 * mu * (1 - voidage) ^ 2 / (dp * dp * voidage ^ 3);
        Real Epsilon = (1 - voidage) / voidage;
        Real[Nc] Km = k_m * (L / Uref);
        Real Kappa = Kozney * L * Uref / Pref;
        Real Pe = L * Uref / Diff;
        Real[Nc] B = b * Cref;
        Real[Nc] Qs = qs / Cref;
        Real Tb = Tbed / Tref;
      ...
      model Adsorber  
        ...
        replaceable package Medium = .Modelica.Media.Interfaces.PartialMixtureMedium;
        .Modelica.Fluid.Interfaces.FluidPort inlet(redeclare package Medium = Medium, Xi_outflow(start = Medium.reference_X[1:Medium.nXi]));
        .Modelica.Fluid.Interfaces.FluidPort outlet(redeclare package Medium = Medium, Xi_outflow(start = Medium.reference_X[1:Medium.nXi]));
      ...
      equation
        ...
        inlet.m_flow = sign(u[N - 1]) * bedParams.Uref * bedParams.csArea * bedParams.voidage * (max(u[N - 1], 0) * Medium.density(inlet_inState) + max(-u[N - 1], 0) * Medium.density(inlet_outState));
        outlet.m_flow = -sign(u[N]) * bedParams.Uref * bedParams.csArea * bedParams.voidage * (max(-u[N], 0) * Medium.density(outlet_inState) + max(u[N], 0) * Medium.density(outlet_outState));
  ...
  package Media  "All fluids that ThermoS knows about" 
    package MyAir  "Specific ideal Gas Mixture of N2 and O2" 
      extends .Modelica.Media.IdealGases.Common.MixtureGasNasa(data = {.Modelica.Media.IdealGases.Common.SingleGasesData.N2, .Modelica.Media.IdealGases.Common.SingleGasesData.O2}, fluidConstants = {.Modelica.Media.IdealGases.Common.FluidData.N2, .Modelica.Media.IdealGases.Common.FluidData.O2}, substanceNames = {"Nitrogen", "Oxygen"}, reducedX = true, reference_X = {0.768, 0.232}, Density(start = 1, nominal = 1), AbsolutePressure(start = 1e5, min = 1e3, max = 50e5, nominal = 1e5), Temperature(start = 300, min = 200, max = 2000, nominal = 300), ThermodynamicState(p(start = 1e5), T(start = 300), X(start = reference_X)), MassFraction(each start = 0.5), SpecificEnthalpy(start = 300e3, nominal = 300e3));
      constant MoleFraction[2] reference_Xm = {0.79, 0.21};
    end MyAir;
  end Media;
