...
//  import Modelica.Media.Interfaces.Types.* ;
  ...
  RealValve vin (redeclare  package Medium = Medium, 
                      cv = 1.4 * 22.36e-8, vchar = Vchar.Linear,  tau = 2, dpTol=0.02) ;
  RealValve vout (redeclare  package Medium = Medium, 
                     cv = 1.12 * 0.9*   1.066*0.33 * 1.15*2.236e-8,  vchar = Vchar.Linear,  tau = 2, dpTol=0.02) ;
  RealValve vblow (redeclare  package Medium = Medium, 
                     cv =  1.2*2*0.07*89.44e-8,  vchar = Vchar.Linear,  tau = 2, dpTol=0.02) ;

  constant Integer CycleTime = 250 ; // Cycle time in sec
