...
//  import Modelica.Media.Interfaces.Types.* ;
  ...
  RealValve vout (redeclare  package Medium = Medium, 
                     cv =  0.5 * 1.15*2.236e-8,  vchar = Vchar.Linear,  tau = 2, dpTol=0.02) ;
  RealValve vblow (redeclare  package Medium = Medium, 
                     cv = 0.6 * 0.25 * 89.44e-8,  vchar = Vchar.Linear,  tau = 2, dpTol=0.02) ;

  constant Integer CycleTime = 160 ; // Cycle time in sec
