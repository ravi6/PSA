...
model bed  
  ...
  .ThermoS.Uops.PSA.Adsorber adsbr(redeclare package Medium = Medium, N = NN, bedParams(Uref = 0.35, L = 0.35, dia = 0.035, voidage = 0.4, Diff = 2.437e-4 / 4.3, dp = 0.7e-3), Coef_p(start = cat(1, {1}, ZEROS)), Coef_Q(start = transpose({CQ1, CQ2})), Coef_y(start = transpose({CY})), Qeq(start = QStart), Q(start = QStart), y(start = yStart), yin_in(start = {0.79, 0.21}), yin_out(start = {0.79, 0.21}));
  constant Real ON = 100;
  constant Real OFF = 100 - ON;
  .ThermoS.Uops.Valves.RealValve vin(redeclare package Medium = Medium, cv = 22.36e-8, vchar = .ThermoS.Uops.Valves.Vchar.Linear, tau = 2, dpTol = 0.02);
  .ThermoS.Uops.Valves.RealValve vout(redeclare package Medium = Medium, cv = 0.5 * 1.15 * 2.236e-8, vchar = .ThermoS.Uops.Valves.Vchar.Linear, tau = 2, dpTol = 0.02);
  .ThermoS.Uops.Valves.RealValve vblow(redeclare package Medium = Medium, cv = 0.6 * 0.25 * 89.44e-8, vchar = .ThermoS.Uops.Valves.Vchar.Linear, tau = 2, dpTol = 0.02);
  constant Integer CycleTime = 160;
...
model plant  
  ...
  constant Real ON = 100;
  constant Real OFF = 100 - ON;
  .ThermoS.Uops.Reservoir src(redeclare package Medium = Medium, p = 4.3e5, T = 300, Xi = Xstart);
  ...
  bed bedA;
  bed bedB;
  .ThermoS.Uops.Valves.RealValve vpurge(redeclare package Medium = Medium, cv = 0.5 * 2.236e-8, vchar = .ThermoS.Uops.Valves.Vchar.Linear, tau = 2, dpTol = 0.02);
initial equation
  bedA.vin.po = OFF;
  bedA.vout.po = ON;
  bedA.vblow.po = OFF;
