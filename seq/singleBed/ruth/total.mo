model bed  
  .ThermoS.Uops.PSA.Adsorber adsbr(redeclare package Medium = .ThermoS.Media.MyAir, N = 12, bedParams(voidage = 0.4, dia = 0.035, L = 0.35, dp = 0.707e-3, Diff = 2.0e-5));
  constant Real ON = 99.9;
  constant Real OFF = 100 - ON;
  .ThermoS.Uops.Valves.RealValve vin(redeclare package Medium = .ThermoS.Media.MyAir, cv = 0.00004 / sqrt(0.5e5), vchar = .ThermoS.Uops.Valves.Vchar.Linear, tau = 100e-3);
  .ThermoS.Uops.Valves.RealValve vout(redeclare package Medium = .ThermoS.Media.MyAir, cv = 4 * 0.000004 / sqrt(0.5e5), vchar = .ThermoS.Uops.Valves.Vchar.Linear, tau = 100e-3);
  .ThermoS.Uops.Valves.RealValve vpurge(redeclare package Medium = .ThermoS.Media.MyAir, cv = 0.00004 / sqrt(0.5e5), vchar = .ThermoS.Uops.Valves.Vchar.Linear, tau = 100e-3);
  constant Integer CycleTime = 100;
  .ThermoS.Uops.PSA.sequencer seq(pTime = 0.3 * CycleTime, fTime = 0.2 * CycleTime, puTime = 0.5 * CycleTime);
initial equation
...
model plant  
  ...
  constant Real ON = 99.9;
  constant Real OFF = 100 - ON;
  .ThermoS.Uops.Reservoir src(redeclare package Medium = .ThermoS.Media.MyAir, p = 3e5, T = 300, Xi = {0.3});
