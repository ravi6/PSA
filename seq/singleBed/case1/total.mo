model bed  
  .ThermoS.Uops.PSA.Adsorber adsbr(redeclare package Medium = .ThermoS.Media.MyAir, N = 12);
  constant Real ON = 99.9;
  constant Real OFF = 100 - ON;
  .ThermoS.Uops.Valves.RealValve vin(redeclare package Medium = .ThermoS.Media.MyAir, cv = 0.04 / sqrt(0.5e5), vchar = .ThermoS.Uops.Valves.Vchar.Linear, tau = 100e-3);
  .ThermoS.Uops.Valves.RealValve vout(redeclare package Medium = .ThermoS.Media.MyAir, cv = 0.004 / sqrt(0.5e5), vchar = .ThermoS.Uops.Valves.Vchar.Linear, tau = 100e-3);
  .ThermoS.Uops.Valves.RealValve vpurge(redeclare package Medium = .ThermoS.Media.MyAir, cv = 0.04 / sqrt(0.5e5), vchar = .ThermoS.Uops.Valves.Vchar.Linear, tau = 100e-3);
  constant Real CycleT = 10;
  .ThermoS.Uops.PSA.sequencer seq(sTime = 0.05, pTime = CycleT * 0.15, puTime = CycleT * 0.15, fTime = CycleT * 0.7);
initial equation
  for n in 1:adsbr.Nc - 1 loop
    for m in 1:adsbr.N - 2 loop
      adsbr.y[m, n] = .ThermoS.Media.MyAir.reference_Xm[n];
    end for;
  end for;
...
model plant  
  constant .ThermoS.Media.MyAir.MassFraction[.ThermoS.Media.MyAir.nXi] Xstart = .ThermoS.Media.MyAir.reference_X[1:.ThermoS.Media.MyAir.nXi];
  constant Real ON = 99.9;
  constant Real OFF = 100 - ON;
  .ThermoS.Uops.Reservoir src(redeclare package Medium = .ThermoS.Media.MyAir, p = 3e5, T = 300, Xi = Xstart);
  .ThermoS.Uops.Reservoir sink(redeclare package Medium = .ThermoS.Media.MyAir, p = 1e5, T = 300, Xi = Xstart);
  .ThermoS.Uops.Reservoir atm(redeclare package Medium = .ThermoS.Media.MyAir, p = 1e5, T = 300, Xi = Xstart);
  bed bedA;
initial equation
  bedA.vin.po = OFF;
  bedA.vout.po = OFF;
  bedA.vpurge.po = ON;
  bedA.seq.pressurizing = false;
  bedA.seq.feeding = false;
  bedA.seq.purging = true;
equation
...
package ThermoS  "A Modelica Package for Process Simulations" 
  package Uops  "Unit Operations in ThermoS Package" 
    package Valves  "Contains Valves and its interfaces " 
      ...
      equation
        ...
        inlet.m_flow = cv * charF * sqrt(abs(Medium.density(state))) * .Modelica.Fluid.Utilities.regRoot(inlet.p - outlet.p, 100);
      end Valve;

      model RealValve  
        extends ThermoS.Uops.Valves.Valve;
        parameter .Modelica.SIunits.Time tau = 0.01;
        .ThermoS.Types.Percent spo;
      equation
        tau * der(po) + po = spo;
      end RealValve;
    ...
    package PSA  "Pressure Swing Adsorption Pakcage" 
      model sequencer  
        parameter .Modelica.SIunits.Time pTime = 5;
        parameter .Modelica.SIunits.Time fTime = 10;
        parameter .Modelica.SIunits.Time puTime = 5;
        parameter .Modelica.SIunits.Time sTime = 1;
        Boolean pressurizing;
        Boolean feeding;
        Boolean purging;
        Boolean pressurize;
        Boolean feed;
      ...
      partial model partAdsorber  
        ...
        constant Real[:] zi = .ThermoS.Math.Chebychev.sTnots(N - 2);
        constant Real[:] z = cat(1, zi, {0, 1});
        constant Real[:, :] vT = transpose(.ThermoS.Math.Chebychev.sT(N, z));
        constant Real[:, :] vTz = transpose(.ThermoS.Math.Chebychev.sTx(N, z));
        constant Real[:, :] vTzz = transpose(.ThermoS.Math.Chebychev.sTxx(N, z));
        type Frac = Real(min = 0, max = 1, nominal = 1, start = 0.1);
        type Vel = Real(min = -130, max = 130, nominal = 0.1, start = 0);
        type Conc = Real(min = 0, max = 100);
        type Coef = Real(min = -120, max = 120);
        type Src = Real(min = -200, max = 200, start = 0);
        type Press = Real(min = 1e-2, max = 20, nominal = 1, start = 1.0);
        Coef[N, Nc - 1] Coef_y;
        Coef[N] Coef_p;
        Coef[N] Coef_u;
        Coef[N, Nc] Coef_Q;
      ...
      equation
        zs = z;
        p = Coef_p * vT;
        u = -1 / bedParams.Kappa * (Coef_p * vTz);
        u = Coef_u * vT;
        ...
        for n in 1:Nc loop
          Q[:, n] = Coef_Q[:, n] * vT;
          for m in 1:N loop
            Qeq[m, n] = max(0, bedParams.Qs[n] * (bedParams.B[n] * p[m] * y[m, n]) / (bedParams.Tb + p[m] * sum(bedParams.B[j] * y[m, j] for j in 1:Nc)));
            S[m, n] = bedParams.Km[n] * (Qeq[m, n] - max(0, Q[m, n]));
          end for;
        end for;
        for n in 1:Nc - 1 loop
          for m in 1:N - 2 loop
            p[m] * (der(Coef_y[:, n]) * vT[:, m] + u[m] * Coef_y[:, n] * vTz[:, m]) - 1 / bedParams.Pe * (p[m] * Coef_y[:, n] * vTzz[:, m] + y[m, n] * (Coef_p * vTzz[:, m] + 2 * (Coef_p * vTz[:, m]) * (Coef_y[:, n] * vTz[:, m]))) + (S[m, n] - y[m, n] * sum(S[m, j] for j in 1:Nc)) * bedParams.Epsilon * bedParams.Tb = 0;
          end for;
        end for;
        for m in 1:N - 2 loop
          der(Coef_p) * vT[:, m] + u[m] * Coef_p * vTz[:, m] + p[m] * (-1 / bedParams.Kappa * Coef_p * vTzz[:, m]) + bedParams.Epsilon * bedParams.Tb * sum(S[m, j] for j in 1:Nc) = 0;
        end for;
        for n in 1:Nc loop
          for m in 1:N loop
            der(Coef_Q[:, n]) * vT[:, m] - S[m, n] = 0;
          end for;
        end for;
        for n in 1:Nc - 1 loop
          Coef_y[:, n] * vTz[:, N - 1] = .Modelica.Fluid.Utilities.regStep(u[N - 1], bedParams.Pe * u[N - 1] * (y[N - 1, n] - yin_in[n]), 0);
          Coef_y[:, n] * vTz[:, N] = .Modelica.Fluid.Utilities.regStep(u[N], 0, bedParams.Pe * u[N] * (y[N, n] - yin_out[n]));
        end for;
        Coef_p[:] * vT[:, N - 1] = p_in;
        Coef_p[:] * vT[:, N] = p_out;
      end partAdsorber;
      ...
      record bedParams  
        constant Integer Nc = 2;
        constant Real R = 8314.5;
        constant Real Pref = 1e5;
        constant Real Tref = 300;
        constant Real Uref = 1.0;
        constant Real Diff = 1e-3;
        constant Real L = 0.25;
        constant Real mu = 2.0e-5;
        constant Real dp = 1.0e-3;
        constant Real dia = 0.2;
        constant Real csArea = 3.147 * dia * dia / 4;
        constant Real Tbed = 300;
        constant Real[Nc] qs = {5.26, 5.26};
        constant Real[Nc] hk = {14.8, 4.7};
        constant Real[Nc] k_m = {19.7, 62.0};
        constant Real voidage = 0.5;
        constant Real Cref = Pref / (R * Tref);
        constant Real[Nc] b = hk ./ qs;
        constant Real Kozney = 180 * mu * (1 - voidage) ^ 2 / (dp * dp * voidage ^ 3);
        constant Real Epsilon = (1 - voidage) / voidage;
        constant Real[Nc] Km = k_m * (L / Uref);
        constant Real Kappa = Kozney * L * Uref / Pref;
        constant Real Pe = L * Uref / Diff;
        constant Real[Nc] B = b * Cref;
        constant Real[Nc] Qs = qs / Cref;
        constant Real Tb = Tbed / Tref;
      end bedParams;

      model Adsorber  
        extends partAdsorber;
        replaceable package Medium = .Modelica.Media.Interfaces.PartialMixtureMedium;
        .Modelica.Fluid.Interfaces.FluidPort inlet(redeclare package Medium = Medium);
        .Modelica.Fluid.Interfaces.FluidPort outlet(redeclare package Medium = Medium);
        Medium.ThermodynamicState inlet_inState;
        Medium.ThermodynamicState outlet_inState;
        Medium.ThermodynamicState inlet_outState;
        Medium.ThermodynamicState outlet_outState;
        constant Medium.MolarMass[Medium.nX] Mw = Medium.fluidConstants[:].molarMass * 1000;
      equation
        sum(yin_in) = 1.0;
        sum(yin_out) = 1.0;
        ...
        inlet.m_flow = u[N - 1] * bedParams.Uref * bedParams.csArea * .Modelica.Fluid.Utilities.regStep(u[N - 1], Medium.density(inlet_inState), Medium.density(inlet_outState));
        outlet.m_flow = -u[N] * bedParams.Uref * bedParams.csArea * .Modelica.Fluid.Utilities.regStep(-u[N], Medium.density(outlet_outState), Medium.density(outlet_inState));
  ...
  package Media  "All fluids that ThermoS knows about" 
    package MyAir  "Specific ideal Gas Mixture of N2 and O2" 
      extends .Modelica.Media.IdealGases.Common.MixtureGasNasa(data = {.Modelica.Media.IdealGases.Common.SingleGasesData.N2, .Modelica.Media.IdealGases.Common.SingleGasesData.O2}, fluidConstants = {.Modelica.Media.IdealGases.Common.FluidData.N2, .Modelica.Media.IdealGases.Common.FluidData.O2}, substanceNames = {"Nitrogen", "Oxygen"}, reducedX = true, reference_X = {0.768, 0.232}, Density(start = 1, nominal = 1), AbsolutePressure(start = 1e5, nominal = 1e5, min = 1.0e3, max = 20e5), Temperature(min = 200, max = 2000, nominal = 300, start = 300), ThermodynamicState(p(start = 1e5), T(start = 300), X(start = reference_X)));
      constant MassFraction[2] reference_Xm = {0.79, 0.21};
    end MyAir;
  end Media;
  ...
  package Math  "All of Math Functionality here " 
    package Chebychev  "Package to compute chebychev polynomial values, derivatives and roots" 
      ...
      algorithm
        y := zeros(n);
        for k in 1:n loop
          y[k] := cos((2 * k - 1) * .Modelica.Constants.pi / (2 * n));
        end for;
      end Tnots;
      ...
      package Common  "Common packages and data for the ideal gas models" 
        ...
        package Functions  "Basic Functions for ideal gases: cp, h, s, thermal conductivity, viscosity" 
          ...
          function dynamicViscosityLowPressure  "Dynamic viscosity of low pressure gases" 
            ...
            input Interfaces.PartialMedium.DipoleMoment mu "Dipole moment of gas molecule";
          ...
          function thermalConductivityEstimate  "Thermal conductivity of polyatomic gases(Eucken and Modified Eucken correlation)" 
            extends Modelica.Icons.Function;
            input Interfaces.PartialMedium.SpecificHeatCapacity Cp "Constant pressure heat capacity";
            ...
            input IdealGases.Common.DataRecord data "Ideal gas data";
            output Interfaces.PartialMedium.ThermalConductivity lambda "Thermal conductivity [W/(m.k)]";
          algorithm
        ...
        partial package MixtureGasNasa  "Medium model of a mixture of ideal gases based on NASA source" 
          ...
          function lowPressureThermalConductivity  "Return thermal conductivities of low-pressure gas mixtures (Mason and Saxena Modification)" 
            ...
            input ThermalConductivity[:] lambda "Thermal conductivities of the pure gases";
          ...
          algorithm
            T := 300;
            for i in 1:100 loop
              dT := 0.1 * (h - h_TX(T, Xfull)) / (h_TX(T + 0.1, Xfull) - h_TX(T, Xfull));
              T := T + dT;
              if abs(dT) < 0.01 then
                break;
              else
            ...
            strX := " ";
            for i in 1:nX loop
              strX := strX + String(Xfull[i]) + " ";
            end for;
            strX := "[" + strX + "]";
            assert(abs(dT) < 0.01, "T_hX unconverged \n" + " h = " + String(h) + " X = " + strX + " T = " + String(T) + " dT = " + String(dT), AssertionLevel.warning);
