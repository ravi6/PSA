...
//  import Modelica.Media.Interfaces.Types.* ;
  ...
  RealValve vin (redeclare  package Medium = Medium, 
                      cv = 0.04/sqrt(0.5e5), vchar = Vchar.Linear,  tau = 100e-3) ;
  RealValve vout (redeclare  package Medium = Medium, 
                     cv = 0.004/sqrt(0.5e5),  vchar = Vchar.Linear,  tau = 100e-3) ;
  RealValve vpurge (redeclare  package Medium = Medium, 
                     cv = 0.04/sqrt(0.5e5),  vchar = Vchar.Linear,  tau = 100e-3) ;
  constant Real CycleT=10 ;
  sequencer seq(sTime=0.05, pTime=CycleT*0.15, puTime=CycleT*0.15, fTime=CycleT*0.7) ;
