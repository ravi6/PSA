...
model plant  
  ...
  constant Real[2] AirComp = Gas.reference_X[1:2];
  .ThermoS.Uops.Reservoir src(redeclare package Medium = Gas, p = 3e5, T = 300, Xi = AirComp);
  .ThermoS.Uops.Reservoir sink(redeclare package Medium = Gas, p = 2e5, T = 300, Xi = AirComp);
  .ThermoS.Uops.Reservoir atm(redeclare package Medium = Gas, p = 1e5, T = 300, Xi = AirComp);
  .ThermoS.Uops.Tanks.GasTank TankA(redeclare package Medium = Gas, vol = 1.0, m(start = 1), Xi(start = AirComp));
  .ThermoS.Uops.Tanks.GasTank TankB(redeclare package Medium = Gas, vol = 1.0, m(start = 1), Xi(start = AirComp));
  .ThermoS.Uops.Valves.RealValve[3] vA(redeclare each package Medium = Gas, each vchar = .ThermoS.Uops.Valves.Vchar.Linear, each tau = 10e-3);
  .ThermoS.Uops.Valves.RealValve[3] vB(redeclare each package Medium = Gas, each vchar = .ThermoS.Uops.Valves.Vchar.Linear, each tau = 10e-3);
  .ThermoS.Uops.Valves.RealValve ve(redeclare each package Medium = Gas, each vchar = .ThermoS.Uops.Valves.Vchar.Linear, each tau = 10e-3);
  sequencer seqA;
  sequencer seqB;
...
initial equation
  ...
  TankB.T = 300;
  TankB.Xi = AirComp;
  seqA.pressurizing = false;
  seqA.feeding = true;
  seqA.purging = false;
  seqB.pressurizing = false;
  seqB.feeding = false;
  seqB.purging = true;
  ve.po = zero;
equation
  ...
  connect(vB[3].outlet, atm.port);
  connect(TankA.outlet, ve.inlet);
  connect(TankB.outlet, ve.outlet);
  ve.spo = 25.0;
  ...
  package Uops  "Unit Operations in ThermoS Package" 
    ...
    package Tanks  " Tanks in ThermoS Package" 
      model GasTank  
        ...
        .Modelica.Fluid.Interfaces.FluidPort inlet(redeclare package Medium = Medium);
        .Modelica.Fluid.Interfaces.FluidPort outlet(redeclare package Medium = Medium);
        parameter .Modelica.SIunits.Volume vol = 10;
        parameter .Modelica.SIunits.EnthalpyFlowRate Q_in = 0;
