...
// This is the cleanest way of have start values for the medium calculations
  ...
  constant Real AirComp[2] = Gas.reference_X[1:2];
  ...
  GasTank  TankA (redeclare package Medium = Gas, vol = 1.0, m(start=1), Xi(start=AirComp));
  GasTank  TankB (redeclare package Medium = Gas, vol = 1.0, m(start=1), Xi(start=AirComp));
  ...
  RealValve ve (redeclare each package Medium = Gas, 
                     each vchar = Vchar.Linear, each tau = 10e-3) ;
