model plant  
  package Medium = ThermoS.Media.MyAir(Density(nominal = 1.0), MassFlowRate(nominal = 1));
  constant .Modelica.Media.Interfaces.Types.MassFraction[1] Xstart = {0.79};
  .ThermoS.Uops.Reservoir src(redeclare package Medium = Medium, p = 5e5, T = 300, Xi = Xstart);
  .ThermoS.Uops.Reservoir sink(redeclare package Medium = Medium, p = 1e5, T = 300, Xi = Xstart, port(m_flow(start = 0.0)));
  .ThermoS.Uops.Valves.RealValve v1(redeclare package Medium = Medium, cv = 1 / sqrt(0.5e5));
  .ThermoS.Uops.Valves.RealValve v2(redeclare package Medium = Medium, cv = 1 / sqrt(0.5e5));
  .ThermoS.Uops.Tanks.Cvolume buffer(redeclare package Medium = Medium, port(m_flow(start = 1.0, nominal = 1)), vol = 1);
initial equation
  buffer.p = 1e5;
  buffer.T = 300;
  buffer.Xi = Xstart;
  ...
  package Uops  "Unit Operations in ThermoS Package" 
    package Valves  "Contains Valves and its interfaces " 
      ...
      model Valve  
        ...
        .ThermoS.Types.Percent po(start = 1);
    ...
    package Tanks  " Tanks in ThermoS Package" 
      ...
      equation
        ...
        der(m * h) = port.m_flow * actualStream(port.h_outflow) + vol * der(p);
        port.Xi_outflow = Xi;
        port.h_outflow = h;
        port.p = p;
      end Cvolume;
