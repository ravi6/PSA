...
model plant  
  ...
  constant Medium.MolarMass[Medium.nS] Mw = Medium.fluidConstants[:].molarMass * 1000;
  constant Medium.MassFraction[Medium.nXi] Xstart = Medium.reference_X[1:Medium.nXi];
  constant Boolean withValves = false;
  Reservoir res1(redeclare package Medium = Medium, T = 300, Xi = Xstart);
  Reservoir res2(redeclare package Medium = Medium, T = 300, Xi = Xstart);
  .ThermoS.Uops.Valves.RealValve v1(redeclare package Medium = Medium, cv = 0.04 / sqrt(0.5e5)) if withValves;
  .ThermoS.Uops.Valves.RealValve v2(redeclare package Medium = Medium, cv = 0.04 / sqrt(0.5e5)) if withValves;
  constant Integer NN = 6;
  constant Integer[:] ZEROS = zeros(NN - 1);
  .ThermoS.Uops.PSA.Adsorber adsorber(redeclare package Medium = Medium, N = NN, bedParams(L = 0.35, dia = 0.035, voidage = 0.4, Uref = 0.35, Diff = 1e-3, dp = 0.7e-3), Coef_p(start = cat(1, {1}, ZEROS)), Coef_Q(start = transpose({cat(1, {11.2}, ZEROS), cat(1, {00.7}, ZEROS)})), Coef_y(start = transpose({cat(1, {0.79}, ZEROS)})));
  Real eps;
  Real voidage;
initial equation
  if withValves then
  ...
  package Uops  "Unit Operations in ThermoS Package" 
    ...
    package PSA  "Pressure Swing Adsorption Pakcage" 
      partial model partAdsorber  
        ...
        Coef[N] Coef_u;
        Coef[N, Nc] Coef_Q;
        Frac[N, Nc] y(each stateSelect = StateSelect.always);
        Vel[N] u;
        Press[N] p(each stateSelect = StateSelect.always);
        Conc[N, Nc] Q(each stateSelect = StateSelect.always);
        Conc[N, Nc] Qeq;
        Src[N, Nc] S;
        Frac[Nc] yin_in;
        Frac[Nc] yin_out;
