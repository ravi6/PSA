...
#figure('Position',[10,10,4000,6200]);

xp = [0:0.02:1];

subplot(3,2,1); hold on ;
ylabel("O2 Mole fraction in gas");
for i = 1:Ns
   #x = asort(D(i).adsorber.zs) ;  y = asort(D(i).y(:,1)')' ;
   #plot(x,y,'LineWidth',2,'color',colors(i),'Marker', 'o', '.'); 
   yp = polyfunc(D(i).adsorber.Coef_y(:,1)', xp)  ;
   plot(xp, 1-yp, 'LineWidth', 2,'color',colors(i), '-'); 
...
for i = 1:Ns
  # x = asort(D(i).adsorber.zs) ;  y = asort(D(i).Q(:,1)')' ;
  # plot(x,y,'LineWidth',2,'color',colors(i),'Marker', 'o', '.'); 
   yp1 = polyfunc(D(i).adsorber.Coef_Q(:,1)', xp);  yp2 = polyfunc(D(i).adsorber.Coef_Q(:,2)', xp);  yp=yp1./(yp1+yp2);
   plot(xp, yp, 'LineWidth', 2,'color',colors(i), '-'); 
   ltxt{i} =  sprintf("t=%.1f",D(i).time);
end
legend(ltxt);
xlabel("Length (dimensionless)");
...
#   plot(x,y,'LineWidth',2,'color',colors(i),'Marker', 'o', '.'); 
   yp = polyfunc(D(i).adsorber.Coef_u, xp);  
   plot(xp, yp, 'LineWidth', 2,'color',colors(i), '-'); 
   ltxt{i} =  sprintf("t=%.1f",D(i).time);
end
legend(ltxt);
...
for i = 1:Ns
   x = asort(D(i).adsorber.zs) ;  y = asort(D(i).adsorber.S(:,1)')' ;
   #plot(x,y,'LineWidth',2,'color',colors(i),'Marker', 'o', '.'); 
...
ylabel("Pressure (bar)");
for i = 1:Ns
   yp = polyfunc(D(i).adsorber.Coef_p, xp);  
   plot(xp, yp, 'LineWidth', 2,'color',colors(i), '-'); 
   ltxt{i} =  sprintf("t=%.1f",D(i).time);
end
legend(ltxt);
...
for i = 1:Ns
yn(i) =  D(i).adsorber.p_in;  
xn(i) =  D(i).time;
end
   plot(xn, yn, 'LineWidth', 2,'color',colors(1), '-'); 
xlabel("time");
grid on ;
...
for i = 1:Ns
yn1(i) =  D(i).adsorber.inlet.m_flow;  
yn2(i) =  -D(i).adsorber.outlet.m_flow;  
xn(i) =  D(i).time;
end
   hold on ;
