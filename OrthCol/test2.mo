...
//    We now work in shifted domain 0<x<1 hence all this rigmorle on x

constant Real [:] xi = ( Tnots(N-2) .+ 1 ) ./ 2    ;  // Interior Collocation Points
constant Real [:] x  = cat(1, xi, {0, 1}) ;  // Including boundaries
constant Real [:,:] vT  =  T(N, (2 * x .- 1) )   ;  // Cheby values (index j) at x (index i)
constant Real [:,:] vTx  = 2 * Tx(N, ( 2 * x .- 1) )   ;  // first deriv Cheby values (index j) at x (index i)
constant Real [:,:] vTxx = 4 * Txx(N, ( 2 * x .- 1))   ;  // second deriv Cheby values (index j) at x (index i)
...
//         Exact Solution  exp(((2x-1)^4-1)/4)
 //       let   u =  sum(Coef[i].T[i])   be the trial solution
    ...
    algorithm  // this recovers u from the Coef's determined earlier
     xs := linspace(0, 1.0, M) ;
     u  := T(N, ( 2 * xs .- 1) ) * Coef   ;  // be careful order of multiplication is important
